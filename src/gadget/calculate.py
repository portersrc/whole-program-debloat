...
parser.add_argument("--D", help="Look at Groupd D benchmarks of Security", action='store_true')
...
parser.add_argument("--bicsa", help="Produce Gadget results for BICSA Code", action='store_true')
parser.add_argument("--bicsa_linker", help="Produce Gadget results for BICSA Code with Linker Script", action='store_true')

args = parser.parse_args()

all_spec_benchs = ["500.perlbench_r", "505.mcf_r", "508.namd_r", "510.parest_r", "511.povray_r", "519.lbm_r", "520.omnetpp_r", "523.xalancbmk_r", "525.x264_r", "526.blender_r", "531.deepsjeng_r", "538.imagick_r", "541.leela_r", "544.nab_r", "557.xz_r"]
all_security_benchs = ["bzip2", "gzip", "tar", "grep", "chown", "date", "mkdir", "rm", "sort", "uniq"]
print_benchs = []
curr_benchs = []
...
groupD = ["bzip2", "gzip", "grep", "tar"]
groupE = ["508.namd_r","557.xz_r", "519.lbm_r"]
maxROP = {}
maxCOP = {}
maxJOP = {}
maxOther = {}
...
if(args.D):
    ...
    print_benchs = all_security_benchs 
if(args.E):
...
if(args.base):
    for bench in curr_benchs:
        ...
        while(line):
            ...
            ROPmin = min(ROPmax, split[0])
            JOPsum += split[1]
            JOPmax = max(JOPmax, split[1])
            JOPmin = min(JOPmax, split[1])
            COPsum += split[2]
            COPmax = max(COPmax, split[2])
            COPmin = min(COPmax, split[2])
            Othersum += split[3]
            Othermax = max(Othermax, split[3])
            Othermin = min(Othermax, split[3])
            Totalsum += split[4]
            Totalmax = max(Totalmax, split[4])
            Totalmin = min(Totalmax, split[4])
            count += 1
            line = f.readline()
        maxROP[bench] = ROPmax
        minROP[bench] = ROPmin
...
if(args.bicsa):
    for bench in curr_benchs:
        f = open("resultswpd_bicsa/"+bench+"/GadgetCounts_Reduction.csv", 'r')
        line = f.readline()
        line = f.readline().strip()
        split = line.split(",")[1:]
        first[bench] = bench + "," + ",".join(split) + "\r"
    ...
    with open("bicsa.csv", 'w') as write:
        write.write(output)
...
if(args.bicsa_linker):
    for bench in curr_benchs:
        f = open("resultswpd_cl_bicsa/"+bench+"/GadgetCounts_Reduction.csv", 'r')
        line = f.readline()
        line = f.readline().strip()
        split = line.split(",")[1:]
        first[bench] = bench + "," + ",".join(split) + "\r"
    ...
    with open("bicsa_linker.csv", 'w') as write:
        write.write(output)