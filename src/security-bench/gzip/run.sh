...
if [ "$2" == "small" ]; then
    ...
    { time ./${BIN} -c < aaa.txt > tmp.log; } &> small-${WHICH}.out
    ...
    if [ "$1" == "base_ls" ]; then
        for run in {1..10}; do
            { /usr/bin/time -v /bin/bash -c './gzip -c -f <sample1.ref >sample1.ref.gz'; } &>> large-${WHICH}.out
            cat debrt.out &>> large-${WHICH}.debrt
        done
    elif  [ "$1" == "artd_release" ]; then
        #for run in {1..10}; do
            { /usr/bin/time -v /bin/bash -c './gzip_artd_release -c -f <sample1.ref >sample1.ref.gz'; } &>> large-${WHICH}.out
            cat debrt.out &>> large-${WHICH}.debrt
        #done
    else
