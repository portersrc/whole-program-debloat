...
if [ "$2" == "small" ]; then
    ...
    touch file
    { time ./${BIN} file; } &> small-${WHICH}.out

elif [ "$2" == "large" ]; then
    rm -f large-${WHICH}.out
    rm -f large-${WHICH}.debrt

	mkdir -p "/tmp/rm_benchmark/dir1/dir2/dir3/dir4"
	touch "/tmp/rm_benchmark/file0.txt" \
		"/tmp/rm_benchmark/file1.txt" \
		"/tmp/rm_benchmark/dir1/file1.txt" \
		"/tmp/rm_benchmark/dir1/dir2/dir3/dir4/file4.txt"

    # BIN isn't working correclty inside /bin/bash -c i guess... hack this for now
    if [ "$1" == "base_ls" ]; then
        for run in {1..10}; do
            { /usr/bin/time -v /bin/bash -c './rm -rf "/tmp/rm_benchmark/dir1/dir2" && \
	./rm -rf "/tmp/rm_benchmark/dir1/file1.txt" && \
	./rm -rf "/tmp/rm_benchmark" && \
	./rm -rf "/tmp/rm_benchmark/dir1/dir2"'; } &>> large-${WHICH}.out
            cat debrt.out &>> large-${WHICH}.debrt
        done
    elif  [ "$1" == "artd_release" ]; then
        #for run in {1..10}; do
            { /usr/bin/time -v /bin/bash -c './rm_artd_release -rf "/tmp/rm_benchmark/dir1/dir2" && \
	./rm_artd_release -rf "/tmp/rm_benchmark/dir1/file1.txt" && \
	./rm_artd_release -rf "/tmp/rm_benchmark" && \
	./rm_artd_release -rf "/tmp/rm_benchmark/dir1/dir2"'; } &>> large-${WHICH}.out
            cat debrt.out &>> large-${WHICH}.debrt
        #done
    else
