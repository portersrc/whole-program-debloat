...
if [ "$2" == "small" ]; then
    ...
    rm -f file1
    touch file1
    { time ./${BIN} rudy:rudy file1; } &> small-${WHICH}.out
    ...
    if [ "$1" == "base_ls" ]; then
        for run in {1..10}; do
            { /usr/bin/time -v /bin/bash -c './chown rudy:rudy file1 && ./chown rudy:rudy d1/d1/d1/file'; } &>> large-${WHICH}.out
            cat debrt.out &>> large-${WHICH}.debrt
        done
    elif  [ "$1" == "artd_release" ]; then
        #for run in {1..10}; do
            { /usr/bin/time -v /bin/bash -c './chown_artd_release rudy:rudy file1 && ./chown_artd_release rudy:rudy d1/d1/d1/file'; } &>> large-${WHICH}.out
            cat debrt.out &>> large-${WHICH}.debrt
        #done
    else
