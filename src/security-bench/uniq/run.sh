...
if [ "$2" == "small" ]; then
    ...
    { time ./${BIN} alphabet.txt; } &> small-${WHICH}.out
    ...
    if [ "$1" == "base_ls" ]; then
        for run in {1..10}; do
            { /usr/bin/time -v /bin/bash -c './uniq -i input.txt >/dev/null'; } &>> large-${WHICH}.out
            cat debrt.out &>> large-${WHICH}.debrt
        done
    elif  [ "$1" == "artd_release" ]; then
        #for run in {1..10}; do
            { /usr/bin/time -v /bin/bash -c './uniq_artd_release -i input.txt >/dev/null'; } &>> large-${WHICH}.out
            cat debrt.out &>> large-${WHICH}.debrt
        #done
    else
