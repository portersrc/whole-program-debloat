...
parser.add_argument("set_folder", help="Folder containing the files of set of pages being mapped at a given time", type=str)
...
if(args.set_folder[-1] != "/"):
    args.set_folder = args.set_folder + "/"

files = glob.glob(args.set_folder+"debrt-mapped-rx-pages_*_wpd_cl_ics.out")

for f in files:
    
    debrt_file = f.split("/")[-1]
    fd = debrt_file.split("_")[1]
    ...
    with open(set_file,"r") as f:
        ...
        while(line):
            ...
            if(args.binary_version == "wpd" or args.binary_version == "wpd_bisca" or args.binary_version == "wpd_ics" or args.binary_version == "wpd_cl_ics"):
    ...
    try:
        if args.result_folder_name is None:
            directory_name = create_output_directory(directory+"analyzer_results/"+fd)
        else:
            directory_name = create_output_directory(directory + args.result_folder_name + "/" + fd, False)
