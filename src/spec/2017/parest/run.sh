...
if [ "$1" == "base" ]; then
    ...
    BIN=510.parest_r_wpd_base
    WHICH=wpd
elif [ "$1" == "wpd_sink" ]; then
    BIN=510.parest_r_wpd_sink
    WHICH=wpd_sink
elif [ "$1" == "wpd_ics" ]; then
    BIN=510.parest_r_wpd_ics
    WHICH=wpd_ics
elif [ "$1" == "wpd_bicsa" ]; then
    BIN=510.parest_r_wpd_bicsa
    WHICH=wpd_bicsa
elif [ "$1" == "wpd_cl" ]; then
    BIN=510.parest_r_wpd_custlink
    WHICH=wpd_cl
elif [ "$1" == "wpd_cl_sink" ]; then
    BIN=510.parest_r_wpd_custlink_sink
    WHICH=wpd_cl_sink
elif [ "$1" == "wpd_cl_ics" ]; then
    BIN=510.parest_r_wpd_custlink_ics
    WHICH=wpd_cl_ics
elif [ "$1" == "wpd_cl_ics_split" ]; then
    BIN=510.parest_r_wpd_custlink_ics_split
    WHICH=wpd_cl_ics_split
elif [ "$1" == "wpd_cl_bicsa" ]; then
    BIN=510.parest_r_wpd_custlink_bicsa
    WHICH=wpd_cl_bicsa
elif [ "$1" == "wpd_ls" ]; then
